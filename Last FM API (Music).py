...
api_key = "ea0f2bf4822ddc2d384a7409fd4d7700"
...
url = "https://ws.audioscrobbler.com/2.0/?method=artist.search&artist=lil&api_key=" + api_key + "&format=json&limit=50"
...
for artist in data['results']['artistmatches']['artist']:
    print(f"{artist['name']} has {artist['listeners']}")
...
url_mbid = "http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&mbid=c9cd225b-4883-428e-82c2-73e0b6282fb6&api_key=" + api_key +"&format=json"


# In[56]:


response_mbid = requests.get(url_mbid)
data_mbid = response_mbid.json()


# In[60]:


data_mbid['artist']['name']


# In[63]:


data_mbid['artist']['bio']['summary']
...
endpoints = []
for x in range(len(mbid_list)):
    url_loop = "http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=" + api_key +"&format=json&mbid=" + mbid_list[x]
    endpoints.append(url_loop)
print(endpoints)
...
for endpoint in endpoints[:3]:
    response_loop = requests.get(endpoint)
    data_loop = response_loop.json()
    print(data_loop['artist']['name'])
...
for endpoint in endpoints[:3]:
    ...
    data_loop = response_loop.json()
    print(data_loop['artist']['name'])
    for tag in data_loop['artist']['tags']['tag']:
        print(tag['name'])
    print("------")
...
for endpoint in endpoints[:10]:
    ...
    data_loop = response_loop.json()
    for tag in data_loop['artist']['tags']['tag']:
        if tag['name'] == 'rap' or 'Hip Hop' and data_loop['artist']['name'] not in rappers:
            rappers.append(data_loop['artist']['name'])
print(rappers)
...
if "rap" or "Hip-Hop" in data_loop['artist']['tags']['tag']:
